...
| ~~1.3~~ ✅ | ~~**The Sun**: HMI continuum granulation texture, limb darkening from a fit, corona (Baumbach model, streamers from noise) visible when the disc is occluded or off-frame, prominences at the limb, lens flare from a ghost/halo model replacing the streak sprite. Retune HDR peak so the disc is a disc, not a white hole.~~ Built as: today's SDO/HMI flattened continuum (fetched with the clouds, reprojected onto the rotating globe for its timestamp) for the spots and faculae; procedural granulation that resolves up close; Neckel & Labs limb darkening; resolved-disc radiance judged against the frame height (the pixel test made a white hole under DLSS); Baumbach K corona and prominences that appear as the disc is covered; five lens ghosts. Parker placed on its Horizons elements and visited at its latest perihelion (clock moved in the cut, brought home in the next), framed from behind so the 2.3 m shield eclipses an 11.7° Sun with a sliver clearing the rim. | 1 | Parker stop: disc with granulation; corona visible past the limb; flare ghosts move opposite the Sun. |
