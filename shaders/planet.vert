...
layout(location = 5) flat out float vBoost; // >1: the sphere is drawn this many times larger than it is
...
void main() {
    ...
    if (shell == 0.0 && type != 5) {
        ...
        local *= 1.0015;
        // A body smaller than a couple of pixels would flicker as the sub-pixel jitter hits and misses it:
        // draw it at a minimum size and let the fragment shader scale its radiance down to conserve flux.
        float radPx = pc.params.w > 0.0 ? b.posRadius.w / max(length(b.posRadius.xyz), 1e-30) / pc.params.w : 1e9;
        const float kMinPx = 1.6;
        if (radPx < kMinPx) {
            boost = kMinPx / max(radPx, 1e-6);
            local *= boost;
        }
    }
    vBoost = boost;
